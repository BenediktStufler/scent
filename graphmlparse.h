...
// We temporarily store the graph in this data structure
...
/*
 * implements a simple hashmap and provides a function that creates
 * a graph out of vertices and edges that are a given by strings and
 * pairs of strings
 */
#include "strtograph.h"
...
struct graph *parsegraphml(char *infile)
{
	char Buff[8192];
	int done;
	int len;
	 ...
	 */
	G = make_graph(status.H);	// recall that some edges of H may point to
